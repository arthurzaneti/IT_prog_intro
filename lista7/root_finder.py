...
class RealFunction:
    ...
    def __init__(self, f = None, prime = None, domain = None):
        self.f = f
        self.prime = prime
        self.domain = domain
...
def newton_root(f: RealFunction, 
    ...
    for i in range(int(maxItr)):
        ...
        if abs(dfx) < 1e-4:
            raise ValueError("Método instavel pois a derivada está proxima de zero")
        
        x_new = x - fx / dfx
        if x_new not in search_space:
            raise ValueError("Método não convergiu")
        
        if abs(x_new - x) < erroTol:
            return x_new
        x = x_new
    return x
